#!/usr/bin/env bash
ps aux|grep "python -m celery_distribute_crawler.myserver"|grep -v grep|tee -a worker_restart.log
ps aux|grep "python -m celery_distribute_crawler.myserver"|grep -v grep|cut -c 10-15|xargs kill -9
